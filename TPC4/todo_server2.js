		...
		case "GET":
			...
			axios.get("http://localhost:3000/users")
				...
				.catch(error => {
					...
					res.end('<p>Erro no acesso à base de dados de users:' + error + '</p>')
				})
			if (req.url == "/")
			{
				axios.get("http://localhost:3000/tasks")
					.then((resp) => {
						...
						res.writeHead(200,  {'Content-Type':'text/html ; charset="utf-8"'})
						res.end(my_pages.genPage(tasks, users))
						})
					.catch((error) => {
						res.writeHead(200,  {'Content-Type':'text/html ; charset="utf-8"'})
						res.end('<p>Erro no acesso à base de dados de tarefas:' + error + '</p>')
					})
			}
			else if (req.url.match(/\/edit\/t\d+/))
			{
				...
				axios.get("http://localhost:3000/tasks")
					.then((resp) => {
						...
						res.writeHead(200,  {'Content-Type':'text/html ; charset="utf-8"'})
						res.end(my_pages.genPage(tasks, users, task))
						})
					.catch((error) => {
						res.writeHead(200,  {'Content-Type':'text/html ; charset="utf-8"'})
